...
REFAPPNAME = TruexRokuReferenceApp
IMPORTS =
ROKU_TEST_ID = 1
...
deploy: $(REFAPPNAME)
	...
	aws s3 cp dist/apps/${REFAPPNAME}.zip s3://$$S3_BUCKET/roku/v${MAJOR}_${MINOR}/${RC_DEVELOP}/${REFAPPNAME}-${RC_DEVELOP}-v${MAJOR}.${MINOR}.${BUILD_NUM}-${BUILD_HASH}.zip --acl public-read ;\
